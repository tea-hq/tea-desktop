...
const text = ref('')
const attachments = ref([{ id: 'brief', name: 'product-requirements-with-a-long-name.md', size: 4096 }])
const activeTurn: ConversationTurn = {
  id: 'turn-1',
  user: { id: 'prompt-1', text: 'Review the drawer interaction and identify any usability risks.', attachments: [] },
  blocks: [{ kind: 'assistantText', id: 'block-1', sequence: 1, text: 'The drawer keeps the Channel canvas focused while preserving Agent context.', streaming: false }],
...
<template>
  <Story title="Agent/Conversation surface" group="Agent">
    ...
    <Variant title="Failure and retry">
      <div class="h-[640px] w-[480px]"><AgentConversationSurface v-model:text="text" v-model:attachments="attachments" :profile="drawerAgentProfile" title="Failed session" :turns="[activeTurn]" error="The Runtime connection was interrupted." :runtimes="[runtime]" :runtime-id="runtime.id" :model-options="[{ value: 'default' }]" model="default" permission-mode="default" /></div>
    </Variant>
    <Variant title="Long Chinese content">
      <div class="h-[720px] w-[480px]"><AgentConversationSurface v-model:text="text" v-model:attachments="attachments" :profile="drawerAgentProfile" title="跨团队协作与桌面端 Agent 会话体验一致性评审" subtitle="产品设计与客户端联合工作组" :turns="[activeTurn]" :runtimes="[runtime]" :runtime-id="runtime.id" :model-options="[{ value: 'default', label: '默认模型与超长本地化展示名称' }]" model="default" permission-mode="default" /></div>
    </Variant>
  </Story>
</template>
