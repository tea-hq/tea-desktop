...
<template>
  <div ref="container" class="agent-thread min-h-0 flex-1 overflow-y-auto px-4 py-4">
    <div v-if="hasOlder" class="flex justify-center pb-3">
      <TeaButton size="small" appearance="ghost" :loading="loadingOlder" @click="loadOlder">{{ t('channels.history.loadMore') }}</TeaButton>
    </div>
    <TeaMessageBar v-if="error" tone="error"><span>{{ error }}</span><TeaButton size="small" appearance="ghost" @click="emit('retry')">{{ t('sidebar.retry') }}</TeaButton></TeaMessageBar>
    <div v-if="loading && turns.length === 0" class="flex h-full items-center justify-center" aria-busy="true"><span class="i-mdi-loading size-5 animate-spin" /></div>
    <TeaEmptyState v-else-if="turns.length === 0" :title="t('messages.empty')" icon="i-mdi-message-outline" />
...
<style scoped>.agent-thread { background: var(--p-surface-0); scrollbar-width: thin; }</style>
