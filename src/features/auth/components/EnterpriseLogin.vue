...
defineProps<{ domain: string; phase: CenterAuthPhase; pending: boolean; errorCode: string | null }>()
  ...
  <main class="flex min-h-screen flex-col tea-bg-canvas tea-fg">
    <header class="flex h-16 shrink-0 items-center px-6 sm:px-10">
      <div class="flex items-center gap-3">
        <span class="flex size-8 items-center justify-center tea-radius-control tea-bg-inverse tea-font-ui tea-text-body tea-weight-strong tea-fg-inverse">T</span>
    ...
    <section class="flex flex-1 items-center justify-center px-6 pb-20 pt-8">
      <form class="w-full max-w-[460px]" @submit.prevent="emit('submit')">
        <p class="tea-text-caption tea-weight-strong uppercase tea-fg-subtle">{{ t('auth.enterprise.eyebrow') }}</p>
        <h1 class="mt-3 tea-font-ui tea-text-display tea-weight-strong leading-tight tea-fg">{{ t('auth.enterprise.title') }}</h1>
        <p class="mt-4 max-w-md tea-text-body leading-6 tea-fg-muted">{{ t('auth.enterprise.description') }}</p>

        <label class="mt-10 block">
          <span class="tea-text-body tea-weight-strong tea-fg">{{ t('auth.enterprise.domainLabel') }}</span>
        ...
        <p v-if="errorCode" class="mt-3 tea-bg-danger-subtle px-4 py-3 tea-text-body tea-fg-danger" role="alert">
        ...
        <div class="mt-6 flex gap-3">
          ...
          >
            <span>{{ phase === 'recoveryRequired' ? t('auth.enterprise.signInAgain') : t('auth.enterprise.continue') }}</span>
