...
export const useCredentialsStore = defineStore('credentials', () => {
  ...
  function configure(nextClient: CredentialClient): void { client = nextClient }

  async function initialize(): Promise<void> {
    if (!client || loading.value) return
    loading.value = true
    error.value = null
    try { records.value = await client.list() } catch { error.value = 'management.credentials.loadFailed' }
  ...
  async function save(mutation: CredentialMutation): Promise<boolean> {
    ...
    try {
      ...
      const index = records.value.findIndex(record => record.pluginId === next.pluginId && record.connectionId === next.connectionId)
      if (index === -1) records.value.push(next)
      else records.value[index] = next
      return true
    } catch { error.value = 'management.credentials.saveFailed'; return false }
  ...
  async function clear(pluginId: string, connectionId: string): Promise<boolean> {
    ...
    try {
      ...
      const record = records.value.find(item => item.pluginId === pluginId && item.connectionId === connectionId)
      if (record) record.configured = false
      return true
    } catch { error.value = 'management.credentials.clearFailed'; return false }
  }
