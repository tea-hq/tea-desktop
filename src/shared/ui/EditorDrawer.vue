...
<template>
  <Transition name="drawer"
    ><aside
      v-if="open"
      class="absolute inset-y-0 right-0 z-20 flex w-full max-w-xl flex-col bg-white shadow-2xl"
    ...
    >
      <header class="flex shrink-0 items-center justify-between px-6 py-5">
        <div>
          ...
          <h2 class="mt-1 text-lg font-semibold text-gray-900">{{ title }}</h2>
        </div>
        <button
          type="button"
          class="inline-flex size-8 items-center justify-center rounded-lg text-gray-400 hover:bg-gray-100 hover:text-gray-700"
      ...
      <footer class="flex shrink-0 justify-end gap-2 bg-gray-50 px-6 py-4">
        <slot name="footer" />
      </footer></aside
  ></Transition>
</template>
