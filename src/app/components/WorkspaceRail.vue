...
<template>
  <nav
    class="hidden h-full w-14 shrink-0 flex-col items-center border-r border-line-soft bg-panel py-3 text-dim sm:flex"
  >
    <TeaIconButton
      ...
      :class="
        activeMode === 'profile'
          ? 'bg-canvas ring-2 ring-fg ring-offset-2 ring-offset-panel'
          : 'hover:bg-canvas'
      "
    ...
    >
      ...
      <span
        v-else
        class="flex size-full items-center justify-center rounded-full bg-canvas text-xs font-semibold text-fg"
      >
        {{ initials }}
      </span>
    </TeaIconButton>
      ...
      <TeaIconButton
        ...
        :class="
          activeMode === entry.mode
            ? 'bg-canvas text-fg before:absolute before:-left-2.5 before:h-4 before:w-0.5 before:rounded-full before:bg-fg'
