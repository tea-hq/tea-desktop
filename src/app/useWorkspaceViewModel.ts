...
import { teaModelOptions } from '@/features/conversation/modelOptions'
...
export function useWorkspaceViewModel(stores: TeaDesktopStores, ui: WorkspaceUiState) {
  ...
  const roleOptions = computed<AgentRoleOption[]>(() => agentRoles.roles
    .filter(role => role.enabled)
    .map(role => ({ id: role.id, name: role.name, revision: role.revision, runtimeId: role.runtimeId })))
  const activeAgentDrawerState = computed(() => channels.activeChannel && collaboration.activeBinding
    ? agentDrawer.ensureState(collaboration.activeBinding)
    : null)
  const collaborationDraft = computed(() => collaboration.activeBinding
    ? agentDrawer.ensureState(collaboration.activeBinding).draft
    : null)
  const fullComposerText = computed({
    get: () => ui.collaborationWorkspace.value ? collaborationDraft.value?.text ?? '' : ui.localComposerText.value,
    set: value => {
      if (ui.collaborationWorkspace.value && collaboration.activeBinding) agentDrawer.updateDraft(collaboration.activeBinding, { text: value })
      else ui.localComposerText.value = value
    },
  })
  const fullComposerAttachments = computed({
    get: () => ui.collaborationWorkspace.value ? collaborationDraft.value?.attachments ?? [] : ui.localComposerAttachments.value,
    set: value => {
      if (ui.collaborationWorkspace.value && collaboration.activeBinding) agentDrawer.updateDraft(collaboration.activeBinding, { attachments: value })
  ...
  const dialogDraft = computed(() => collaboration.collaboration.drafts.find(
    value => value.draftId === ui.draftDialogId.value,
  ) ?? null)
  const dialogDelivery = computed(() => dialogDraft.value
    ? collaboration.collaboration.deliveries.find(value =>
  ...
  const currentChannelSessionAvailable = computed(() => {
    ...
    return Boolean(active?.channelBinding && binding
  ...
  const collaborationModelOptions = computed<ModelOption[]>(() => teaModelOptions(
    collaboration.activeRuntime,
    collaboration.selectedModel,
