...
function registerIpc(): void {
  ipcMain.handle(
    "tea:command",
    async (_event, command: unknown, args: unknown) => {
      if (!isDesktopCommand(command))
        throw { code: "unsupportedCommand", retryable: false };
...
async function handleCommand(
  ...
  switch (command) {
    case "resolve_center_enterprise":
      return requireCenterAuthService().resolveEnterprise(
        readString(args.domain, "domain"),
      );
    case "start_center_login":
      return requireCenterAuthService().startLogin(
        readString(args.domain, "domain"),
      );
      ...
      return (
        ...
        requireManagedWorkspaceService()
          ...
          .then(() => ({
            ...
            defaultEnterpriseDomain: normalizeDomain(
              process.env["TEA_CENTER_ENTERPRISE_DOMAIN"],
            ),
          }))
      );
    ...
    case "list_conversations":
      return requireConversationService().listConversations(
        readRecord(args.request) as never,
      );
    ...
    case "load_conversation_history":
      return requireConversationService().loadHistory(
        readRecord(args.request) as never,
      );
    ...
    case "prepare_draft_delivery":
      return requireConversationService().prepareDelivery(
        readString(args.draftId, "draftId"),
      );
    ...
    case "send_message":
      return requireConversationService().send(
        ...
        {
          model:
            args.model === null ? "default" : readString(args.model, "model"),
          permissionMode: readPermissionMode(args.permissionMode),
        },
        Array.isArray(args.sources) ? (args.sources as never) : [],
      );
    case "cancel_conversation":
      return requireConversationService().cancel(
        readString(args.conversationId, "conversationId"),
      );
    ...
    case "resolve_host_tool_call":
      return requireConversationService().resolveHostToolCall(
    ...
    case "archive_conversation":
      return requireConversationService().archive(
        readString(args.conversationId, "conversationId"),
      );
    case "delete_conversation":
      return requireConversationService().remove(
        readString(args.conversationId, "conversationId"),
      );
    case "list_plugins":
      return requireCatalogService().listPlugins();
    case "enable_plugin":
      return requireCatalogService().setPluginEnabled(
    ...
    case "disable_plugin":
      return requirePluginProcessService()
        ...
        .then(() =>
          requireCatalogService().setPluginEnabled(
            readString(args.pluginId, "pluginId"),
            false,
          ),
        );
    ...
    case "save_plugin_credentials":
      return requireCredentialService().save(
        readRecord(args.mutation) as never,
      );
    ...
    case "open_direct_conversation":
      return requireChannelService().openDirectConversation(
        readString(args.accountId, "accountId"),
      );
    ...
    case "mark_channel_read":
      return requireChannelService().markRead(
...
function readPermissionMode(
  value: unknown,
): "default" | "readOnly" | "fullAccess" {
  if (value === "default" || value === "readOnly" || value === "fullAccess")
    return value;
  throw {
    code: "invalidRequest",
    retryable: false,
...
function readApprovalDecision(
  value: unknown,
): "allowOnce" | "allowSession" | "deny" | "cancel" {
  if (
    value === "allowOnce" ||
    value === "allowSession" ||
    value === "deny" ||
...
app.whenReady().then(() => {
  ...
  centerAuthService = new ElectronCenterAuthService(
    ...
    (state) =>
      win?.webContents.send("tea:event:center-auth-state-changed", state),
  );
  conversationService = new ElectronConversationService(
    ...
    (summary) =>
      win?.webContents.send("tea:event:conversation:updated", summary),
    (call) =>
      win?.webContents.send("tea:event:conversation:host-tool-call", call),
  );
  ...
  pluginProcessService = new ElectronPluginProcessService(
    catalogService,
    credentialService,
  );
