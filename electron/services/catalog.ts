...
import type { AgentRoleDependency, AgentRoleRecord } from '../../src/features/agent-roles/contracts'
...
export class ElectronCatalogService {
  ...
  async setPluginEnabled(pluginId: string, enabled: boolean): Promise<void> {
    const plugin = this.state.plugins.find((value) => value.id === pluginId)
    if (!plugin) throw serviceError('invalidRequest', false, `unknown plugin: ${pluginId}`)
  ...
  async saveAgentRoleRevision(input: AgentRoleRevisionInput): Promise<AgentRoleRecord> {
  ...
  async syncAgentRoles(request: { tenantId: string; subjectId: string }): Promise<{
...
function normalizeRole(input: AgentRoleRevisionInput): AgentRoleRecord {
  if (
    ...
    throw serviceError('invalidRequest', false, 'agent role revision is invalid')
  }
  const capabilities = (input.capabilities ?? []).map((value) => normalizeCapability(value))
  const dependencies = input.dependencies.map((value) => {
    ...
    throw serviceError('invalidRequest', false, 'agent role dependency is incomplete')
  })
  return {
    id: input.roleId,
    ...
    visibility: validVisibility(input.visibility) ? input.visibility : 'private',
...
function parseRemoteRoleResponse(value: unknown, tenantId: string): Array<Record<string, unknown>> {
  ...
  return value.roles.map((role) => {
    ...
    return {
      ...
      currentRevision: {
        ...
        userPromptTemplate:
          typeof revision.userPromptTemplate === 'string' ? revision.userPromptTemplate : undefined,
...
function errorCode(error: unknown): string {
  return isRecord(error) && typeof error.code === 'string' ? error.code : 'centerUnavailable'
}

function normalizeCapability(value: unknown): NonNullable<AgentRoleRecord['capabilities']>[number] {
  if (!isRecord(value))
...
function validVisibility(value: string | undefined): value is 'tenant' | 'restricted' | 'private' {
