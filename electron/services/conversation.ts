...
class CodexAppServer {
  ...
  private constructor(
    ...
    const handleExit = (code: number | null, cause?: Error) => {
      ...
      const error = cause ?? new Error(`Codex app-server exited with code ${code ?? 'unknown'}`)
...
export class ElectronConversationService {
      ...
      for (const turn of conversation.turns) {
        if (turn.status === 'sending' || turn.status === 'running') {
          ...
          turn.blocks.push({
            ...
            failure: {
              code: 'unavailable',
              message: 'The previous runtime process ended before the app reopened.',
              retryable: true,
            },
          })
        }
    ...
    return [
      {
        ...
        status: resolveExecutable(process.env['TEA_CLAUDE_EXECUTABLE'] || 'claude')
          ? 'ready'
          : 'unavailable',
        models: [],
      },
      {
        ...
        displayName: 'Codex',
        capabilities: [...STANDARD_CAPABILITIES],
        status: resolveExecutable(process.env['TEA_CODEX_EXECUTABLE'] || 'codex')
          ? 'ready'
          : 'unavailable',
        models: [],
      },
  ...
  private handleCodexValue(value: Record<string, unknown>): void {
    ...
    if (method === 'item/agentMessage/delta') {
      if (typeof params.delta === 'string') this.appendDelta(conversation, turn, params.delta)
      return
    }
    ...
    if (method === 'item/completed') {
      ...
      if (tool) this.appendToolCompleted(conversation, turn, tool.id, tool.status, tool.message)
      return
    }
    if (method === 'turn/completed') {
      const status = readStringValue(
        params.turn && isRecord(params.turn) ? params.turn.status : params.status,
      )
  ...
  private async handleCodexRequest(
    ...
    const conversation = threadId
      ...
      : undefined
    if (!conversation || !threadId) throw new Error('Codex request belongs to an unknown thread')
    const conversationId = conversation.summary.conversationId
    ...
    if (
      ...
      return this.waitForCodexApproval(value, conversation, threadId, method, params)
    }
  ...
  private waitForCodexApproval(
    ...
    const permissions =
      kind === 'permissions' && isRecord(params.permissions)
  ...
  async listConversations(request: ListConversationsRequest): Promise<ConversationPage> {
    ...
    const items = this.state.conversations
      ...
      .filter((value) => matchesFilter(value.summary, request.filter ?? { kind: 'all' }))
      .sort(
        (left, right) =>
  ...
  async loadHistory(request: LoadConversationHistoryRequest): Promise<ConversationHistoryPage> {
    if (!request.conversationId.trim())
      throw serviceError('invalidRequest', false, 'conversationId must not be empty')
    if (!Number.isInteger(request.limit) || request.limit < 1 || request.limit > MAX_HISTORY_LIMIT)
    ...
    const end = request.cursor ? decodeCursor(request.cursor) : conversation.turns.length
  ...
  async createConversation(
    ...
    if (!runtime) throw serviceError('invalidRequest', false, `unknown runtime: ${runtimeId}`)
    if (runtime.status !== 'ready')
      throw serviceError('runtimeUnavailable', true, `${runtime.displayName} is not ready`)
    const now = Date.now()
    ...
    const summary: ConversationSummary = {
      ...
      createdAt: now,
      updatedAt: now,
      ...(channelBinding ? { channelBinding: structuredClone(channelBinding) } : {}),
    }
  ...
  async appendSources(
    ...
    if (
      ...
      throw serviceError('invalidRequest', false, 'conversation sources are invalid')
    const context = conversation.collaboration.turnContexts.find(
      (value) => value.turnIndex === turnIndex,
    )
    if (!context)
      throw serviceError('invalidRequest', false, 'conversation turn context is unknown')
    const existing = new Set(context.sources.map((value) => sourceKey(value.messageRef)))
    const added = sources
  ...
  async updateDelivery(
    ...
    if (sentMessageRef) result.delivery.sentMessageRef = structuredClone(sentMessageRef)
  ...
  async shutdown(): Promise<void> {
    ...
    await Promise.all(
      conversationIds.map((conversationId) => this.cancel(conversationId).catch(() => undefined)),
    )
  ...
  async respondToApproval(
    ...
    if (codexApproval) {
      if (codexApproval.conversationId !== conversationId)
        throw serviceError('invalidRequest', false, 'approval belongs to a different conversation')
    ...
    if (!approval || approval.conversationId !== conversationId)
      throw serviceError('invalidState', false, 'approval request is no longer pending')
    const response =
      decision === 'allowOnce'
        ? {
            behavior: 'allow',
    ...
    if (!response)
      throw serviceError('invalidState', false, 'Claude did not offer a session permission rule')
    try {
  ...
  async resolveHostToolCall(result: HostToolResult): Promise<void> {
    ...
    if (!conversation.hostTools.some((tool) => tool.name === pending.toolName))
      throw serviceError('invalidRequest', false, 'host tool result references an unavailable tool')
    clearTimeout(pending.timer)
  ...
  async send(
    ...
    if (this.activeRuns.has(conversationId))
      throw serviceError('invalidState', true, 'a message is already in progress')
    ...
    try {
      const runtimeText = conversation.summary.channelBinding
        ? buildChannelPrompt(prompt, conversation.collaboration.turnContexts.at(-1)?.sources ?? [])
        : prompt
      ...
      else throw serviceError('runtimeUnavailable', true, 'runtime is not registered')
      if (turn.status === 'sending' || turn.status === 'running') {
  ...
  private async requestHostTool(call: HostToolCall, active: ActiveRun): Promise<HostToolResult> {
  ...
  private appendToolCompleted(
    ...
    const block = turn.blocks.find((value) => value.kind === 'toolCall' && value.id === toolCallId)
  ...
  private async runClaude(
    ...
    const hostToolScope = await this.startClaudeHostToolScope(conversation, active)
    const args = [
      '-p',
      '--input-format',
      'stream-json',
    ...
    if (conversation.turns.length === 1) args.push('--session-id', conversation.nativeSessionId)
    else args.push('--resume', conversation.nativeSessionId)
    if (options.model && options.model !== 'default') args.push('--model', options.model)
    if (options.permissionMode === 'readOnly') args.push('--permission-mode', 'plan')
    else if (options.permissionMode === 'fullAccess') args.push('--dangerously-skip-permissions')
      ...
      await readLines(child, (line) => {
        ...
        if (event.kind === 'delta') {
          ...
          this.appendToolRequested(
            ...
            event.toolCallId,
            event.toolName,
            isRecord(event.input) ? (event.input as Record<string, ConversationJson>) : {},
          )
          this.emitConversationEvent(conversation, turn, {
            type: 'approvalRequested',
            approvalId,
            ...
            decisions: [
              'allowOnce',
              ...(event.permissionSuggestions.length > 0 ? (['allowSession'] as const) : []),
              'deny',
              'cancel',
            ],
          })
        } else if (event.kind === 'finished') turn.status = 'completed'
        else if (event.kind === 'failed') throw serviceError('externalCli', false, event.message)
      })
  ...
  private async startClaudeHostToolScope(
    ...
    const server = createServer((request, response) => {
      void this.handleClaudeMcpRequest(request, response, conversation, active, token).catch(() =>
  ...
  private async handleClaudeMcpRequest(
    ...
    if (value.method !== 'tools/call') {
      writeMcpResponse(response, {
        jsonrpc: '2.0',
        id,
        error: { code: -32601, message: 'method unsupported' },
      })
      return
    }
  ...
  private async runCodex(
    ...
    const threadId = await this.ensureCodexThread(conversation, server, requestedModel)
    ...
    const turnStart: Record<string, unknown> = {
      ...
      cwd: this.workspacePath,
      approvalPolicy: options.permissionMode === 'default' ? 'on-request' : 'never',
  ...
  private findDraft(draftId: string): {
    ...
    for (const conversation of this.state.conversations) {
      const draft = conversation.collaboration.drafts.find((value) => value.draftId === draftId)
  ...
  private findDelivery(deliveryId: string): {
    ...
    throw serviceError('unknownDelivery', false, `unknown delivery: ${deliveryId}`)
  }
  ...
  private requireConversation(conversationId: string): StoredConversation {
    ...
    if (!conversation)
      throw serviceError('unknownConversation', false, `unknown conversation: ${conversationId}`)
    return conversation
  }
...
function requireBoundConversation(
  ...
  if (!conversation.summary.channelBinding)
    throw serviceError('channelBindingUnavailable', false, 'conversation is not bound to a Channel')
...
function buildChannelPrompt(instruction: string, sources: ChannelSource[]): string {
...
function resolveExecutable(value: string): string | undefined {
  ...
  const result = spawnSync(process.platform === 'win32' ? 'where' : 'which', [value], {
    encoding: 'utf8',
  })
...
function extractCodexTurnId(value: Record<string, unknown>): string | undefined {
  ...
  return (
    readStringValue(result?.turnId) || readStringValue(turn?.id) || readStringValue(value.turnId)
  )
}

function extractCodexThreadIdFromNotification(value: Record<string, unknown>): string | undefined {
  const params = isRecord(value.params) ? value.params : undefined
  const turn = params && isRecord(params.turn) ? params.turn : undefined
  return (
    readStringValue(params?.threadId) ||
    readStringValue(params?.thread_id) ||
    readStringValue(turn?.threadId)
  )
}
...
function codexPermissionCapabilities(permissions: Record<string, unknown>): string[] {
...
function codexApprovalResult(
  ...
  return {
    permissions:
      decision === 'allowOnce' || decision === 'allowSession' ? approval.permissions || {} : {},
    scope: decision === 'allowSession' ? 'session' : 'turn',
  }
}

function collectCodexResources(params: Record<string, unknown>, fields: string[]): string[] {
  return [
    ...new Set(
...
function codexErrorMessage(params: Record<string, unknown>): string {
  ...
  return (
    readStringValue(error?.message) || readStringValue(params.message) || 'Codex execution failed'
  )
}
...
function missingSuffix(emittedText: string, snapshot: string): string | undefined {
  ...
  const suffix = snapshot.startsWith(emittedText) ? snapshot.slice(emittedText.length) : ''
...
function parseClaudeLine(line: string):
  ...
  if (
    ...
    return typeof delta.text === 'string' ? { kind: 'delta', text: delta.text } : undefined
  ...
  if (value.type === 'error')
    return {
      kind: 'failed',
      message: typeof value.message === 'string' ? value.message : 'Claude execution failed',
    }
  ...
  if (value.type === 'control_request' && request?.subtype === 'can_use_tool')
    return {
      ...
      toolCallId: String(request.tool_use_id || value.request_id || randomUUID()),
...
async function readRequestBody(request: IncomingMessage, maxBytes: number): Promise<string> {
  ...
  for await (const chunk of request) {
    ...
    if (bytes > maxBytes) throw serviceError('invalidRequest', false, 'MCP request is too large')
    chunks.push(value)
  }
...
function writeMcpResponse(response: ServerResponse, value: Record<string, unknown>): void {
...
async function waitForChild(child: ChildProcessWithoutNullStreams): Promise<void> {
  ...
  if (code !== 0)
    throw serviceError('externalCli', false, `agent CLI exited with code ${code ?? 'unknown'}`)
}
