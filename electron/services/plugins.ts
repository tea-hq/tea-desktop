...
export class ElectronPluginProcessService {
  ...
  async invoke(request: PluginInvocationRequest): Promise<unknown> {
    ...
    const connection = plugin.connections.find((value) => value.id === request.connectionId)
    if (!connection || !connection.enabled)
      throw serviceError('runtimeUnavailable', false, 'plugin connection is unavailable')
    const action = plugin.actions.find((value) => value.id === request.actionId)
    if (!action) throw serviceError('invalidRequest', false, 'plugin action is unknown')
    const credentialValue = await this.credentials.readValue(request.pluginId, request.connectionId)
    if (!credentialValue)
      throw serviceError(
        'runtimeUnavailable',
        false,
    ...
    try {
      ...
      return await process.invoke(action.id, action.version, request.input, credentialValue)
    } catch (error) {
...
class PluginProcess {
  ...
  private async exchange(request: Record<string, unknown>): Promise<PluginResponse> {
    if (!this.isRunning())
      throw serviceError('runtimeUnavailable', true, 'plugin process is not running')
    ...
    const responsePayload = await withTimeout(this.readFrame(), 'read plugin protocol frame')
  ...
  private async readBytes(length: number): Promise<Buffer> {
    while (this.frameBuffer.length < length) {
      ...
      if (next.done)
        throw serviceError('runtimeUnavailable', true, 'plugin process closed its output')
...
function writeFrame(child: ChildProcessWithoutNullStreams, payload: Buffer): Promise<void> {
