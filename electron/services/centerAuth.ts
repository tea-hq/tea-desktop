...
export class ElectronCenterAuthService {
  ...
  async resolveEnterprise(domain: string): Promise<EnterpriseDirectory> {
    ...
    try {
      ...
      if (
        !enterprise.loginAvailable ||
        enterprise.organizationDomain !== normalized
      )
  ...
  async startLogin(domain: string): Promise<CenterAuthState> {
    ...
    try {
      ...
      if (!address || typeof address === "string")
        throw serviceError("invalidCallback", false);
  ...
  async refreshBootstrap(): Promise<CenterAuthState> {
    ...
    try {
      const bootstrap = await this.request<EndpointBootstrap>(
        "/v1/endpoint/bootstrap",
  ...
  private async handleCallback(
    ...
    response.end(
      "<!doctype html><title>Tea</title><p>Authentication received. Return to Tea.</p>",
    );
    this.pending = null;
  ...
  private async refreshSession(refresh: string): Promise<void> {
    const privateKey = this.loadPrivateKey();
    const signature = sign(null, Buffer.from(refresh), privateKey).toString(
      "base64url",
    );
    const session = await this.request<SessionResponse>(
      "/v1/endpoint-sessions/refresh",
      {
        method: "POST",
        body: {
  ...
  private async request<T = unknown>(
    ...
    if (!origin)
      throw serviceError(
        "centerUnavailable",
        true,
        "TEA_CENTER_ORIGIN is not configured",
      );
