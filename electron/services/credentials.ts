...
import type { CredentialMutation, CredentialRecord } from '../../src/features/credentials/contracts'
...
export class ElectronCredentialService {
  ...
  async save(mutation: CredentialMutation): Promise<CredentialRecord> {
    ...
    const entry = {
      ...
      encrypted: safeStorage.encryptString(JSON.stringify(mutation.value)).toString('base64'),
      updatedAt: mutation.updatedAt,
    }
    const index = file.entries.findIndex(
      (value) => value.pluginId === entry.pluginId && value.connectionId === entry.connectionId,
    )
  ...
  async clear(pluginId: string, connectionId: string): Promise<void> {
    ...
    file.entries = file.entries.filter(
      (value) => value.pluginId !== pluginId || value.connectionId !== connectionId,
    )
    await this.write(file)
  }

  async readValue(pluginId: string, connectionId: string): Promise<Record<string, unknown> | null> {
    const file = await this.read()
    ...
    if (!isRecord(value))
      throw serviceError('storageFailure', true, 'credential payload is invalid')
    return value
  }
  ...
  private async read(): Promise<CredentialFile> {
    try {
      ...
      if (
        ...
        throw serviceError('storageFailure', true, 'credential state is invalid')
      throw serviceError('storageFailure', true, 'credential state could not be read')
    }
  }
  ...
  private async write(value: CredentialFile): Promise<void> {
    ...
    try {
      ...
      throw serviceError('storageFailure', true, 'credential state could not be written')
    }
  }
}

function validateKey(pluginId: string, connectionId: string): void {
  if (!/^[A-Za-z0-9._:-]{1,128}$/.test(pluginId) || !/^[A-Za-z0-9._:-]{1,128}$/.test(connectionId))
